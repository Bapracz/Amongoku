...
public class Sudoku : Adw.Application
{
    ...
    private void create_game ()
    {
        backend.generate_game (play_difficulty, (obj) => {
            // Ensure the game view is initialized before attempting to update it.
            // game_view exists from the template, but its init() may not have been called yet,
            // so grid and actions can be null. Use grid==null as a proxy for initialization.
            if (game_view == null || game_view.grid == null)
                start_game ();
            else
            {
